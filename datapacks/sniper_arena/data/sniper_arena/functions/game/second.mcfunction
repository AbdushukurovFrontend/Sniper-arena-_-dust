...
execute if score #state sa.var matches 2 if score #alive sa.var matches ..1 unless score #solo sa.var matches 1 run function sniper_arena:game/not_enough
...
# (mod bo'lsa buni faol arena chegarasi bo'yicha mod qiladi)
execute if score #state sa.var matches 2 unless score #mod sa.var matches 1 as @e[type=minecraft:player,tag=sa.ingame,tag=!sa.deathcam] unless entity @s[x=148,y=-64,z=-89,dx=69,dy=44,dz=95] run function sniper_arena:game/out_of_bounds
