# O'lim ekrani davomida har tick: kamera qotilga qarab turadi, o'yinchi kameraga bog'langan
...
execute if entity @a[tag=sa.kcam] as @e[type=minecraft:armor_stand,tag=sa.mycam] at @s run tp @s ~ ~ ~ facing entity @a[tag=sa.kcam,limit=1] eyes
# Shift yoki sichqoncha bilan kameradan chiqib ketgan bo'lsa qaytariladi (o'zgarmagan bo'lsa hech narsa yuborilmaydi)
